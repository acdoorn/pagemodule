		  ...
		  <div class="contentselect">
		  	...
		  	<select class="form-control" name="content[]" id="choose" style="width:100%;" size="5">
		  		@foreach($modules as $module)
			        ...
			        // Copy the elseif statement below to insert a module that doesn't have any fields

			        /* Copy for editspot 1 */

					// elseif($module->value == /*MODULEVALUE*/) {
					// 	$value.= '<div><h4 class="form-control">/*MODULENAME*/</h4></div>'.Form::hidden(/*MODULEVALUE*/, true, ['class'=>/*MODULEVALUE*/]); 
					// }

					/* End of copy for editspot 1 */

					if($module->value == 'articlelist') {
						$value.= '<div><h4 class="form-control">Articlelist</h4></div>'.Form::hidden('articlelist', true, ['class'=>'articlelist']); 
					}
					/* EDITSPOT 1 */
			  		...
			  		@foreach($fields as $field) 
			  			...
			  			@elseif($fieldtype->name == 'image') 
							<?php $value.= '<div class="input-group">
								...
								?>
						<?php /* Copy for editspot 2, only for standard types that Laravel supports--

				  			@elseif($fieldtype->name == /FIELDTYPE (same as database)/)
								<?php $value.= '<div class="input-group">
								    <span class="input-group-addon">'.ucwords($field->name).'</span>'
									. Form::FIELDTYPE($field->name, Input::old($field->name), ['placeholder' => ucwords($field->name), 'class'=>'form-control', 'style'=>'resize:none;', 'required' => 'required']) .'</div><br/>'?>  
									
									--End of copy for editspot 2 */?>
						{{-- EDITSPOT 2 --}}
